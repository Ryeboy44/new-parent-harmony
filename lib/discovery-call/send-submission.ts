  ...
  const contactLabel =
    data.contactMethod === "email"
      ...
      : data.contactMethod === "phone"
        ...
        : "Email or phone";
  return `
    <h2>New free discovery call request</h2>
...
export async function sendDiscoverySubmission(
  ...
  if (configError) {
    ...
    return {
      ok: false,
      message:
        "We couldn’t send your message right now. Please try again in a few minutes, or refresh the page and submit the contact form again.",
    };
  }
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: config.to,
      replyTo: data.email,
      subject: `Discovery call request: ${data.fullName}`,
      html: buildHtml(data),
    });
